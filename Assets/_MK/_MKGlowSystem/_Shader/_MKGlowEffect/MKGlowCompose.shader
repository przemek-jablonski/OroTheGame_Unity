	...
	Subshader 
	{
		...
		Pass 
		{
			...
			fixed4 frag( Output i ) : SV_TARGET
			{
				...
				//special thanks to Jakob Thomsen
				fixed d = (min(0,_GlowTex_TexelSize.y)/_GlowTex_TexelSize.y)*(1 - i.uv[1].y) + (max(0,_GlowTex_TexelSize.y)/_GlowTex_TexelSize.y)*(i.uv[1].y);
